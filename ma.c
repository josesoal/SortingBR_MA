/*
 ============================================================================
 Project    : Memetic Algorithm for the SUPBR Problem
 File       : ma.c
...
static int start_ma(parameters *params, int *num_eval_fit_function, int *num_used_generations);
...
int main(int argc, char **argv){
    ...
    score = start_ma(&params, &num_eval_fit_function, &num_used_generations);
...
static int start_ma(parameters *params, int *num_eval_fit_function, int *num_used_generations){
    ...
    LS_for_population(&pop,1,num_eval_fit_function);
    ...
    if (start_breeding == TRUE){
        for(generation_number=2; generation_number<=number_of_generations; generation_number++){
            ...
            if (ma_reached_degenerate_state(&pop)){
                restart_population_LS(&pop, &perm,num_eval_fit_function);
            }
